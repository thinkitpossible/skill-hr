...
**P02 only:** build recall shortlist, score installed candidates, emit **`delegate` / `confirm` / `recruit`** with conformant JSON. You do not install skills, write P03, or run debrief.
...
- `references/10-multi-skill-agent.md` — matching against employee bundles, task-type / closure context
...
2. **P02b**: Score each candidate with subscores that sum to the total; document **hard_negative_explanations** for runners-up **only when** `recall_shortlist` has ≥2 candidates (otherwise omit the field or use `[]`—no fabricated runner-ups). Assign **`confidence`** using P02’s calibration table and thin-excerpt cap.
